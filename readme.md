...
# 3结束
感觉QT还是非常的好入门的，只要是想认真搞这个课设的，稍微花一点时间还是会很轻松的拿下的，需要用到什么功能可以参考我的博客。附上自己的整个工程的代码还有报告，里面还有个可以直接运行的程序（不用QT就可以直接运行），当然为了避免白嫖，我会收点小费用的（哈哈，码字费）。链接如下：[数控课设](https://download.csdn.net/download/newpeopie/86028670)
